...
def main():
	...
	# risk data in some postal code areas might be undefined (NaNs or -1.0)
	# In these areas, there are either no inhabitants at all, or too few inhabitans
	# so the risk data is not shown for privacy reasons
	# Set the risk in these regions to the national average
	data.replace(-1.0, np.nan, inplace=True)
	avg_risk = np.nanmean(data['risk'])
	data.fillna(avg_risk, inplace=True)

	# Add column relative median risk (w.r.t national average)
	...
	data = data.round({'pop_density': 2, 'risk_relative': 2})
	...
	# Assign colors to zip codes based on relative median risk by binning data
